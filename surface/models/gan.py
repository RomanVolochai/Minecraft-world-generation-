...
class PatchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        return torch.as_tensor(self.patches[idx], dtype=torch.long)
...
class GANModel:
    def __init__(self, latent_dim=100, num_epochs=5, batch_size=32, lr=0.0002, num_classes=24):
    ...
    def _to_one_hot(self, patches):
        ...
        patches_tensor = patches.to(self.device, dtype=torch.long, non_blocking=True)
    ...
    def fit(self, train_patches):
        print("Preparing GAN dataset (integer patches, lazy one-hot per batch)...")
        ...
        dataset = PatchDataset(train_patches)
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, data in enumerate(loop):
                real_data = self._to_one_hot(data)
